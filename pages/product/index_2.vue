        ...
        <div class="section-fluid pt-0">
            <div class="banner-product" style="display: flex; align-items: center; height: 350px; background: url(/images/banner-product.png) no-repeat; background-size: cover; background-position: 30%">
                <div class="container" v-if="!isLoadingCategory">
                    <h1 class="text-black mb-1 text-uppercase text-primary">{{ categories.length ? "Lihat Semua Produk" : categories?.name }}</h1>
                    <ul class="breadcrumb mb-0 pt-0">
                        ...
                        <template v-else>
                            <li class="current">{{ categories.length ? "Lihat Semua Produk" : categories?.name }}</li>
                        </template>
                    </ul>
                </div>
            ...
            <div class="container mt-5">
                <div class="row">
                    <div class="col-12">
                        <p v-if="isLoading" class="mb-4">Memproses, Silahkan Menunggu...</p>
                        <div class="d-flex flex-wrap align-items-center justify-content-between mb-4" style="gap: 12px" v-else>
                            <div class="d-flex flex-wrap align-items-center mb-3 mb-lg-0" style="gap: 12px">
                                <h3 class="text-black mb-0">{{ categories.length ? (keyword ? `Search Result "${keyword}"` : "Lihat Semua Produk") : keyword ? `Search Result "${keyword}"` : subCategoryId ? subcategories?.filter((el) => el.id == subCategoryId)[0].name : categories?.name }}</h3>
                                <button class="btn btn-sm btn-primary px-2" v-if="keyword" @click="clearKeyword()">Hapus kata kunci</button>
                            </div>
                            <div class="d-flex align-items-center">
                                ...
                                <select class="form-control" v-model="sortType" @change="onChangeSort" style="min-width: 120px">
                                    ...
                                    <option value="name,asc">Nama dari A-Z</option>
                                    <option value="name,desc">Nama dari Z-A</option>
