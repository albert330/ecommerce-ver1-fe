<template>
    <div>
        <div class="row" v-if="listAddress.length > 0">
            <div :class="col" v-for="item in listAddress" :key="item.id">
                <div class="border rounded-lg position-relative p-3 mb-3" :class="{ 'border-primary bg-transparent-primary': item.id == (type == 'shipping' ? selectedAddressId : selectedBillingId) }">
                    <button class="btn position-absolute p-0" @click="handleDelete(item.id)" style="right: 1rem; top: 1rem">
        ...
        <b-modal id="modalConfirm" title="Confirmation" size="md" centered>
            <template #default>
                <p class="mb-0">Are you sure to delete this address ?</p>
            </template>
            <template #modal-footer>
                <div class="d-flex align-items-center">
                    <button class="btn btn-md btn-info mr-3" @click="closeModal()">Cancel</button>
                    <button class="btn btn-md btn-danger" @click="removeAddress()" >Delete</button>
                </div>
            </template>
        </b-modal>
    </div>
